...
import com.wust.ws.wsokhttp.FileDownloadActivity;
import com.wust.ws.wsokhttp.R;
...
public class InterceptFileDownloadActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_intercept_file_download);
    ...
    private void initOkhttp() {
        Interceptor interceptor = new Interceptor() {
            ...
            public Response intercept(Chain chain) throws IOException {
                ...
                return  response.newBuilder().body(new ProgressResponseBody(response.body(),new Mpro())).build();
            }
        };
        client = new OkHttpClient.Builder().addNetworkInterceptor(interceptor).build();
    }

    class Mpro implements ProgressListener{
        ...
        public void onDone(long totalSize) {
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(InterceptFileDownloadActivity.this,"下载完成",Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
        ...
        switch (requestCode){
            case EXTERNAL_STORAGE_CODE:
            {
                ...
                if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED){
                    //申请成功，进行相应的操作
                    Toast.makeText(InterceptFileDownloadActivity.this,"已授权成功",Toast.LENGTH_SHORT).show();
                }else{
                    //申请失败，可以继续向用户解释
                }
            }
